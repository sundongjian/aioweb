...
def create_pool(loop, **kw):		# 引入关键字后不用显示import asyncio了
    ...
    __pool = yield from aiomysql.create_pool(    #aiomysql异步mysql驱动支持
...
@asyncio.coroutine
def select(sql, args, size=None):#select是选择后输出的模式，所以返回结果集
...
def execute(sql, args, autocommit=True):
    ...
    log(sql)
    with (yield from __pool) as conn:
        if not autocommit:#如果不是自动提交，则手动打开
        ...
        try:
            ...
            if not autocommit:
                yield from conn.commit()#手动提交
        except BaseException as e:
            if not autocommit:
                yield from conn.rollback()
            raise e  # raise不带参数，则把此处的错误往上抛;为了方便理解还是建议加e吧
        return affected#insert,update不需要输出内容
