...
# 是为了验证当前的这个请求用户是 否在登录状态下，或是否是伪造的sha1
    ...
    def auth(request):
        ...
        if cookie_str:
            ...
            if user:
                ...
                logging.info('current user: %s' % request.__user__)
...
def init(loop):
    ...
    add_routes(app, 'handlers')#def add_routes(app, module_name),handlers对应的是处理模块
    # 添加静态文件所在地址
    add_static(app)
    # 启动
